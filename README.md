...
### 核心
- **TCP 连通性测试** — 多线程并发检测（最高 200），实时进度，支持终止
- **超时精度** — 支持小数秒（0.1~60s），适配不同网络环境
- **IP 范围支持** — 单 IP、CIDR (`10.0.0.0/24`)、范围 (`192.168.1.1-10`)、换行分隔
- **端口范围支持** — 单端口、范围 (`1-100`)、逗号 (`80,443`)、换行分隔、混合格式
- **IP × 端口笛卡尔积** — 自动生成所有组合，大数量异步导入带进度条
- **端口扫描** — 扫描指定 IP 范围的端口，发现开放端口后导入集合
- **高并发稳定性** — 批量写库 + 信号线程锁，200 并发不崩溃

### 管理
- **集合管理** — 将目标按场景分组（生产/测试/数据库等），支持批量选中删除
- **拖拽排序** — 集合列表和目标表格均支持拖拽排序
- **筛选过滤** — 所有面板支持文本搜索 + 状态筛选（连通/未连通/未测试），输入防抖优化
- **列排序** — 点击列标题排序，IP 按数字段自然排序，箭头指示升降序
- **列宽自适应** — 序号/端口/状态按内容自适应，IP/描述/集合弹性填充窗口
- **勾选测试** — 勾选部分目标一键测试，自动跳转到测试页
- **批量操作** — 集合和目标均支持多选批量删除

### 导入导出
- **CSV** — 逗号分隔，UTF-8 BOM，兼容 Excel
- **Excel** — `.xlsx` / `.xls`，带样式（表头蓝色、连通绿色、失败红色）
- **IP/端口范围展开** — 导入单元格内支持 CIDR、范围、换行分隔
- **防重** — 以「集合 + IP + 端口」判定重复，支持覆盖已有数据
- **异步导入** — 大数量（>1000 条）二次确认，后台线程导入 + 进度对话框
...
**列顺序：** `IP地址, 端口, 描述, 集合名称`（描述和集合可省略）

#### IP 列支持的格式

| 格式 | 示例 | 展开结果 |
|------|------|----------|
| 单个 IP | `192.168.1.1` | 1 个 |
| CIDR 网段 | `10.0.0.0/30` | 2 个 (`10.0.0.1`, `10.0.0.2`) |
| 完整范围 | `192.168.1.1-192.168.1.3` | 3 个 |
| 简短范围 | `192.168.1.1-3` | 3 个 |
| 换行分隔 | `192.168.1.1`<br>`192.168.1.5` | 2 个 |
| 混合 | 换行 + CIDR + 范围任意组合 | 全部展开 |
...
| 范围 | `1-100` | 100 个 |
| 逗号分隔 | `80,443` | 2 个 |
| 换行分隔 | `80`<br>`443`<br>`8080` | 3 个 |
| 混合 | `80,443`<br>`8000-8010`<br>`22` | 14 个 |

> **笛卡尔积：** IP 列和端口列各自展开后，生成所有组合。
> 如 IP 展开为 3 个、端口展开为 100 个 → 共 300 条目标。
> 超过 1000 条会弹出二次确认，导入过程异步执行并显示进度条。

**CSV 示例：**
```csv
ip,port,description,batch_name
192.168.1.1,80,主 Web 服务器,生产环境
"192.168.1.1-192.168.1.3","80,443",Web 服务,生产环境
"192.168.1.1
10.0.0.1-10.0.0.2","22
8080",管理端口,测试环境
10.0.0.0/29,3306,数据库,生产环境
```

- 第 2 行：3 个 IP × 2 个端口 = 6 条
- 第 3 行：3 个 IP × 2 个端口 = 6 条
- 第 4 行：6 个 IP × 1 个端口 = 6 条

**Excel 用法：** 在单元格内按 `Alt+Enter` 换行输入多个 IP 或端口。
...
- **终止测试**：测试中点击「⏹ 停止测试」可随时中断
- **实时筛选**：结果表格上方支持文本搜索 + 状态筛选，筛选不影响后台完整记录
- **超时设置**：支持小数秒（如 0.5s、1.5s），精确控制检测灵敏度
- **并发控制**：1~200 并发可调，大量目标时建议 50~100 以获得最优性能
- 完成后可到「测试历史」导出筛选结果
